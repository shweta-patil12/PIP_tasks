...
class ProductSpider(scrapy.Spider):
    ...
    def parse_amazon(self, response):
        ...
        product_prices = response.css('div.mr1 mr2-xl b black green lh-solid f5 f4-l span.a-price .a-offscreen::text').extract()

        if not product_links or not product_titles:
            self.logger.warning("No product links or titles found on Amazon page.")
        if not product_prices:
            self.logger.warning("No prices found on Amazon page.")

        for title, link, price in zip(product_titles, product_links, product_prices):
            yield {'product_name': title.strip(), 'product_url': response.urljoin(link), 'product_price': price.strip()}
    ...
    def parse_walmart(self, response):
        ...
        product_prices = response.css('span.w_iUH7::text').extract()  # Scrape the price based on class 'w_iUH7'
    ...
    def parse_flipkart(self, response):
        ...
        self.logger.info(f"Raw HTML: {response.text[:500]}")  # Print the first 500 characters to inspect
        chrome_options = Options()
        chrome_options.add_argument("--headless")  # Run in headless mode (no UI)
        driver = webdriver.Chrome(executable_path='/usr/local/bin/chromedriver', options=chrome_options)  # Provide path to chromedriver

        # Use Selenium to load the page
        driver.get(response.url)
        
        # Wait for content to load (adjust sleep time if needed)
        time.sleep(5)
        ...
        # Convert the rendered HTML into a Scrapy Response object
        ...
        product_prices = response.css('div.Nx9bqj::text').extract()  # This is where we extract the price
    ...
    def parse_nike(self, response):
        ...
        product_prices = response.css('div.product-price.is--current-price.css-1ydfahe::text').extract()  # Adjusted selector for price


        if not product_titles or not product_links:
            self.logger.warning("No product titles or links found on Nike page.")

        for title, link, subtitle, price in zip(product_titles, product_links, product_subtitles, product_prices):
            combined_title = f"{title.strip()} - {subtitle.strip()}"
            yield {'product_name': combined_title, 'product_url': link,
            'product_price': price.strip() if price else 'N/A'}
